...
export default function DashboardClient({ initialContests, initialOffers, referralStats }: DashboardClientProps) {
    ...
    // We can keep connection error handling if we want to support client-side revalidation later,
    // but for now, if server fetch fails, page.tsx handles it or passes empty data.
    // We'll remove the client-side error state for now as initial load is server-side.
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
                ...
                <div className="bg-white p-6 rounded-2xl border border-zinc-100 shadow-sm">
                    ...
                    <div className="space-y-4">
                        {liveContests.length > 0 ? liveContests.map((contest) => (
                            <Link
                                key={contest._id}
                                href={`/dashboard/contest/${contest._id}`} // Use _id from DB
