...
export default function SidebarClient({ initialCredits, userName, role, isCollapsed, toggleSidebar, isMobileOpen = false, setMobileOpen }: SidebarClientProps) {
    ...
    useEffect(() => {
        if (!effectiveCollapsed && !isTourActive && !isMobile) {
    ...
    }, [effectiveCollapsed, toggleSidebar, isTourActive, isMobile]);

    // Reset timer on user interaction (but NOT if modal is open or tour is active)
    const handleInteraction = () => {
        if (!effectiveCollapsed && autoCloseTimerRef.current && !isTourActive && !isMobile) {
            clearTimeout(autoCloseTimerRef.current);
            autoCloseTimerRef.current = setTimeout(() => {
                toggleSidebar();
            }, 5000);
    ...
    return (
        <>
            ...
            <motion.div
                initial={false}
                animate={{
                    width: isMobile ? 288 : (effectiveCollapsed ? 80 : 288),
                    x: isMobile ? (isMobileOpen ? 0 : -300) : 0
                }}
                transition={{
                    duration: effectiveCollapsed ? 0.3 : 0.4,
                    ease: effectiveCollapsed ? [0.32, 0, 0.67, 0] : [0.23, 1, 0.32, 1]
                }}
            ...
            >
                ...
                <div className={`h-16 flex items-center ${effectiveCollapsed ? 'justify-center' : 'justify-between px-6'}`}>
                    {!isMobile && (
                        <button
                        ...
                        >
                            <div className="relative w-6 h-6 flex items-center justify-center">
                                ...
                                <motion.div
                                    animate={{
                                        x: effectiveCollapsed ? 0 : -14,
                                        y: effectiveCollapsed ? -4 : 0,
                                        scale: effectiveCollapsed ? 1 : 1,
                                        rotate: effectiveCollapsed ? 0 : 0
                                    }}
                                ...
                                <motion.div
                                    animate={{
                                        x: effectiveCollapsed ? 4 : 0,
                                        y: effectiveCollapsed ? 4 : 0,
                                        scale: effectiveCollapsed ? 1 : 1,
                                        rotate: effectiveCollapsed ? 120 : 0
                                    }}
                                ...
                                <motion.div
                                    animate={{
                                        x: effectiveCollapsed ? -4 : 14,
                                        y: effectiveCollapsed ? 4 : 0,
                                        scale: effectiveCollapsed ? 1 : 1,
                                        rotate: effectiveCollapsed ? 240 : 0
                                    }}
                    ...
                    <AnimatePresence>
                        {!effectiveCollapsed && (
                            <motion.span
                                ...
                                className={`text-xs font-semibold text-zinc-400 uppercase tracking-wider whitespace-nowrap ${isMobile ? 'ml-2' : ''}`}
                            >
                                Menu
                            </motion.span>
                        )}
                ...
                {role !== 'admin' && (
                    <div className="px-4 mb-6">
                        <motion.div
                            layout
                            className={`bg-white/50 border border-zinc-200/50 rounded-2xl overflow-hidden relative ${effectiveCollapsed ? 'h-14' : 'h-20'}`}
                ...
                <nav className="flex-1 px-3 space-y-1">
                    <NavItem id="nav-dashboard" href="/dashboard" icon={LayoutGrid} label="Dashboard" isCollapsed={effectiveCollapsed} />
                    {role === 'admin' ? (
                        <>
                            <NavItem id="nav-upload" href="/dashboard/upload" icon={BookOpen} label="Upload Solutions" isCollapsed={effectiveCollapsed} />
                            <NavItem id="nav-analytics" href="/dashboard/analytics" icon={BarChart3} label="Analytics" isCollapsed={effectiveCollapsed} />
                            <NavItem id="nav-concerns" href="/dashboard/concerns" icon={MessageSquare} label="User Concerns" isCollapsed={effectiveCollapsed} />
                            <NavItem id="nav-offers" href="/dashboard/offers" icon={Tag} label="Offers & Coupons" isCollapsed={effectiveCollapsed} />
                        </>
                    ) : (
                        <>
                            <NavItem id="nav-solutions" href="/dashboard/solutions" icon={BookOpen} label="My Solutions" isCollapsed={effectiveCollapsed} />
                            <NavItem id="nav-help" href="/dashboard/help" icon={HelpCircle} label="Help & Support" isCollapsed={effectiveCollapsed} />
                        </>
                    )}
                    <NavItem id="nav-settings" href="/dashboard/settings" icon={Settings} label="Settings" isCollapsed={effectiveCollapsed} />
                </nav>
